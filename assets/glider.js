...
function initAutoScrollGlider() {
  glider = new Glider(document.querySelector('.glider'), {
    ...
    let isAutoScrolling = true; // enable auto scrolling

    const autoScrollInterval = 3000; // Interval between auto scrolls

    function autoScroll() {
      if (isAutoScrolling) {
        glider.scrollItem(currentSlide , true); // Scroll to the next item

        currentSlide = currentSlide === 0 ? 3 : 0;
  ...
  // Call the function to initialize Glider with auto scrolling
  initAutoScrollGlider();